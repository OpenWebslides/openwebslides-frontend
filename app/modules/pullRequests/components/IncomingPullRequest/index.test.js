...
import IncomingPullRequests, { PureIncomingPullRequest } from '.';
...
describe(`IncomingPullRequests`, (): void => {
  ...
  it(`loads the pull request, when the upstream pullRequest was not previously present in the state`, (): void => {
    ...
    const enzymeWrapper = mount(
      <DummyProviders dummyState={dummyState} dummyDispatch={dummyDispatch}>
        <IncomingPullRequests pullRequestId={dummyPullRequest.id} />
      </DummyProviders>,
    );

    expect(dummyDispatch).toHaveBeenCalledWith(actions.fetch(dummyPullRequest.id));
    expect(enzymeWrapper.find('[data-test-id="incoming-pull-request"]').hostNodes()).toHaveLength(0);
  });

  it(`renders the incoming pull request component, when the pull request was previously present in the state`, (): void => {
    const enzymeWrapper = mount(
      <DummyProviders dummyState={dummyState} dummyDispatch={dummyDispatch}>
        <IncomingPullRequests pullRequestId={dummyPullRequest.id} />
      </DummyProviders>,
    ...
    expect(enzymeWrapper.find('[data-test-id="incoming-pull-request"]').hostNodes()).toHaveLength(1);
  });
  ...
  it(`renders the pull request message`, (): void => {
    ...
    expect(enzymeWrapper.find('[data-test-id="incoming-pull-request-message"]').hostNodes().html()).toContain(dummyPullRequest.message);
  });
