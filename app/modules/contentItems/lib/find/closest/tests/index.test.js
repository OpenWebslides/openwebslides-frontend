...
describe(`findClosest`, (): void => {
  ...
  it(`returns the first recursive simpleFindFunction result for which the passed predicate returns TRUE`, (): void => {
    const dummyPredicate = (contentItem: ContentItem): boolean => (contentItem.type === contentItemTypes.HEADING);
    const actualResult = findClosest(dummyParagraph1121, dummyContentItemsById, find.parentOrSuperItem, dummyPredicate);
  ...
  it(`returns NULL, when the passed contentItem is NULL`, (): void => {
    const actualResult = findClosest(null, dummyContentItemsById, find.parentOrSuperItem, jest.fn(() => true));
    expect(actualResult).toBeNull();
  });

  it(`calls the passed precicate function with the correct arguments`, (): void => {
    const dummyPredicate = jest.fn(() => false);
    findClosest(dummyParagraph1121, dummyContentItemsById, find.parentOrSuperItem, dummyPredicate);
