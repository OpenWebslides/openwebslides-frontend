...
describe(`findFurthest`, (): void => {
  ...
  it(`returns the last recursive singleFindFunction result, when no predicate is passed`, (): void => {
    const actualResult = lib.find.furthest(dummyParagraph1121, dummyContentItemsById, lib.find.superItem);
  ...
  it(`returns the last recursive singleFindFunction result for which the passed predicate returns TRUE`, (): void => {
    ...
    const actualResult = lib.find.furthest(dummyParagraph1121, dummyContentItemsById, lib.find.superItem, dummyPredicate);
  ...
  it(`returns NULL, when the passed contentItem is NULL`, (): void => {
    const actualResult = lib.find.furthest(null, dummyContentItemsById, lib.find.superItem, jest.fn(() => true));
    expect(actualResult).toBeNull();
  });

  it(`calls the passed precicate function with the correct arguments`, (): void => {
    const dummyPredicate = jest.fn(() => false);
    lib.find.furthest(dummyParagraph1121, dummyContentItemsById, lib.find.superItem, dummyPredicate);
