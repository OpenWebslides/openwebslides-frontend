...
import findLastChildOrSubItem from '..';

describe(`findLastChildOrSubItem`, (): void => {
  ...
  it(`returns the last subItem, if the passed contentItem is a superItem`, (): void => {
    const actualResult = findLastChildOrSubItem(dummyHeading1, dummyContentItemsById);
    const expectedResult = dummyParagraph12;
    expect(actualResult).toBe(expectedResult);
  });

  it(`returns the last childItem, if the passed contentitem is a parentItem`, (): void => {
    const actualResult = findLastChildOrSubItem(dummyRoot, dummyContentItemsById);
    const expectedResult = dummyHeading2;
    expect(actualResult).toBe(expectedResult);
  });

  it(`returns the last subItem, if the passed contentItem is a parentItem as well as a superItem`, (): void => {
    const actualResult = findLastChildOrSubItem(dummyTestParentAndSuperItem21, dummyContentItemsById);
    const expectedResult = dummyParagraph214;
    expect(actualResult).toBe(expectedResult);
  });

  it(`returns NULL, when the passed contentItem is NULL`, (): void => {
    const actualResult = findLastChildOrSubItem(null, dummyContentItemsById);
    expect(actualResult).toBeNull();
  });

  it(`returns NULL, when the passed contentItem is neither a parent nor a superItem`, (): void => {
    const actualResult = findLastChildOrSubItem(dummyParagraph11, dummyContentItemsById);
    expect(actualResult).toBeNull();
  });
  ...
  it(`throws a CorruptedInternalStateError, when the passed contentItemsById object contains inconsistencies`, (): void => {
    ...
    expect((): void => {
      findLastChildOrSubItem(dummyHeading1, dummyContentItemsById);
