...
describe(`RejectPullRequestModal`, (): void => {
  ...
  it(`does not render with currentUserId NULL, when there is no current user`, (): void => {
    ...
    expect(enzymeWrapper.find(`PureRejectPullRequestModal`).props().currentUserId).toBeNull();
  });
  ...
  it(`loads the topics, when the topics or its content were not previously present in the state`, (): void => {
    ...
    expect(enzymeWrapper.find('[data-test-id="reject-pull-request-modal"]').hostNodes()).toHaveLength(0);
  });
  ...
  it(`renders the modal, when the topics were previously present in the state`, (): void => {
    ...
    expect(enzymeWrapper.find('[data-test-id="reject-pull-request-modal"]').hostNodes()).toHaveLength(1);
  });
  ...
  it(`renders the modal form and submit/cancel buttons, when the passed isOpen prop is TRUE`, (): void => {
    ...
    expect(enzymeWrapper.find('[data-test-id="reject-pull-request-modal-submit-button"]').hostNodes()).toHaveLength(1);
    expect(enzymeWrapper.find('[data-test-id="reject-pull-request-modal-cancel-button"]').hostNodes()).toHaveLength(1);
  });
  ...
  it(`calls the passed onSubmit callback when the submit button is clicked`, (): void => {
    ...
    enzymeWrapper.find('[data-test-id="reject-pull-request-modal-submit-button"]').hostNodes().simulate('click');
  ...
  it(`calls the passed onCancel callback when the cancel button is clicked`, (): void => {
    ...
    enzymeWrapper.find('[data-test-id="reject-pull-request-modal-cancel-button"]').hostNodes().simulate('click');
